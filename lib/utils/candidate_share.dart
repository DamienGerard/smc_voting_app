...
class CandidateShare {
  ...
  static Future<List<CandidateShare>> getCandidateSharesByShare(
    ...
    var raw_candidateShares = db
        .select('SELECT * FROM candidate_share WHERE share_id = ?', [share_id]);
  ...
  Future<void> delete() async {
    final db = await getDB();
    try {
      dbDelete('candidate_share',
          {'share_id': share_id, 'candidate_id': candidate_id}, db);
    } on SqliteException {
