...
class Candidate {
  ...
  static Future<List<Candidate>> getCandidatesOfElection(
    ...
    var raw_candidate_list = db.select(
        'SELECT candidate.candidate_id, name, party, tally, local_subTally FROM candidate INNER JOIN election_candidate ON candidate.candidate_id = election_candidate.candidate_id WHERE election_candidate.election_id = ?',
        [election_id]);
  ...
  Future<void> save(String election_id) async {
    ...
    var raw_candidate_list = db.select(
        'SELECT * FROM election_candidate WHERE election_id = ? AND candidate_id = ?',
  ...
  Future<void> delete(String election_id) async {
    final db = await getDB();
    try {
      dbDelete('candidate', {'candidate_id': candidate_id}, db);
      dbDelete('election_candidate',
          {'candidate_id': candidate_id, 'election_id': election_id}, db);
    } on SqliteException {
