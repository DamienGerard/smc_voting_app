...
/*Future<Database> getDB() async {
  open.overrideForAll(_openOnAll);
  Directory appDocDirectory = await getApplicationDocumentsDirectory();
  //WidgetsFlutterBinding.ensureInitialized();
  /*var dbFile = File(appDocDirectory.path + '/voter_$identifier.db');
  ...
  if (!dbFile.existsSync()) {
    ...
    sink.close();
    final dbCreator =
        sqlite3.open(appDocDirectory.path + '/voter_$identifier.db');
    dbCreator.execute(
  ...
  final db = sqlite3.open(appDocDirectory.path + '/voter_$identifier.db');
...
Future<Database> getDB() async {
  ...
  var db = sqlite3.open(appDocDirectory.path + '/voter_$identifier.db');

  return db;
}

DynamicLibrary _openOnAll() {
  ...
  try {
    ...
    appid =
        String.fromCharCodes(appid.codeUnits.where((element) => element != 0));
...
void dbDelete(String tableName, Map<String, dynamic> filterBy, Database db) {
  var numConditions = filterBy.length;
  var conditionStr = '';
  var conditionValues = <dynamic>[];
  filterBy.forEach((columnName, value) {
    conditionStr = conditionStr + ' ' + columnName + ' = ? ';
  ...
  var sqlQuery = 'DELETE FROM ' + tableName + ' WHERE' + conditionStr;
...
Future<String> getLocalID() async {
  final prefs = await SharedPreferences.getInstance();
  return prefs.get('local_nid') as String? ?? '';
}
